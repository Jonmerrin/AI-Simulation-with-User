...
public class Interactions
{
    ...
    public void react(Mover guy, Mover otherGuy){
        ...
        //So... thinking about adding a tag on people to identify the mode. You know, for stuff. So it doesn't always print.
        if(!guy.getMode()){
            System.out.println(lastInteract);
            System.out.println("");
        }
    ...
    public String listResponses(int opt, String aName){
        System.out.println("You can:\n");
        if(opt == 0){
            return("1. Initiate an interaction with "+aName+"\n2.Leave "+aName+ "alone.");
        }
        if(opt == 1){
            return("1. Argue back against "+aName+"\n2.Ignore "+aName);
        }
        else if(opt == 2){
            return("1. Attempt to defend yourself against "+aName+"\n2. Give up your money to "+aName+"\n3. Attempt to flea from "+aName);
        }
        else if(opt == 3){
            return("1. Punch "+aName+" in the face\n2. Punch "+aName+" in the face and storm off\n3. Attempt to aggravate "+aName+" with a witty retort\n4. Attempt to make a joke of it by playing along\n5. Ignore "+aName);
        }
        else if(opt == 4){
            return("1. Respond to "+aName+" with positive conversation\n2. Insult "+aName+"\n3. Ignore "+aName);
        }
        else if(opt == 5){
            return("1. Accept "+aName+"'s moneys graciously\n2.Accept "+aName+"'s moneys begrudgingly\n3.Refuse "+aName+"'s moneys politely.\n4. Refuse "+aName+"'s moneys rudely.");
        }
        else{
            return("1. Participate in the debate and see who wins\n2. Get fed up with and insult "+aName+"\n3. Ignore "+aName);
        }
    }
}
